...
with open(os.path.dirname(__file__)+'/../b_w_lists/guid.json') as file:
    guid_bw = json.load(file)
regex_string = r"\b(([0-9a-fA-F]{8})[-\s]?([0-9a-fA-F]{4}[-\s]?[0-9a-fA-F]{4}[-\s]?[0-9a-fA-F]{4})[-\s]?([0-9a-fA-F]{12}))\b"
tag_name = "GUID"
category_guid = "5af367c3-463e-42f0-89c0-73d54af65c03"

whitelist_words = set(guid_bw["whitelist"])
blacklist_words = set(guid_bw["blacklist"])
words_count = 4
    ...
    def match_regex(self, text: str, sensitive: Optional[bool] = True):
        if text:
            ...
            for match_num, match in enumerate(regex_compiled.finditer(text), 1):
                ...
                if sensitive:
                    result = result[0:2] + "xxxxxx-xxxx-xxxx-xxxx-xxxxxxxxxxxx"
