...
with open(os.path.dirname(__file__)+'/../b_w_lists/cpr.json') as file:
    cpr_bw = json.load(file)

regex_string = r"\b(([0-3][0-9]\s?[0-1][0-9]\s?\d{2})(?:[\s\-/\.\t]|\s\-\s)?(\d{4}))\b"
tag_name = "CPR"
category_guid = "02ff6ae6-58dd-42f7-845c-e142d4e8153a"
whitelist_words = set(cpr_bw["whitelist"])
blacklist_words = set(cpr_bw["blacklist"])
words_count = 4
    ...
    def match_regex(self, text: str, sensitive: Optional[bool] = True):
        if text:
            ...
            for match_num, match in enumerate(regex_compiled.finditer(text), 1):
                ...
                cpr_numbers = match.group(2).replace(" ", "") + match.group(3)
